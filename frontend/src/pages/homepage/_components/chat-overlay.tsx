...
import { startRingTone, stopRingTone } from "@/lib/ring-tone.ts";
import { notifyMissedWakeUp } from "@/lib/push.ts";
...
export default function ChatOverlay({
  ...
  // ── Music (full-screen page; audio lives in a module singleton so playback
  //    never re-renders the chat — see lib/music.ts) ─────────────────────────
  const [musicOpen, setMusicOpen] = useState(false);

  // ── "Wake Me Up" — urgent ring to Faizan's iPhone (native companion app).
  // Plays a soft web ring tone while the overlay shows, and listens on
  // Supabase Realtime for the call being answered/declined so Habiba sees
  // "Faizan answered 💗" live. Gracefully degrades to plain ringing UI until
  // the wake-up SQL (wake_up_requests + request_wake_up) has been run.
  const [wakeRinging, setWakeRinging] = useState(false);
  const [wakeStatus, setWakeStatus] = useState<
    "ringing" | "answered" | "declined" | "noanswer" | "error"
  >("ringing");
  // Human-readable reason when the Twilio call could not be placed
  // (e.g. geo-permissions, trial verification, out of credit).
  const [wakeError, setWakeError] = useState<string | null>(null);
  const wakeTimerRef = useRef<number | null>(null);
  const wakeCloseRef = useRef<number | null>(null);
  const wakePollRef = useRef<number | null>(null);
  ...
  const teardownWake = useCallback(() => {
    ...
    if (wakeCloseRef.current) {
      clearTimeout(wakeCloseRef.current);
      wakeCloseRef.current = null;
    }
    if (wakePollRef.current) {
      clearInterval(wakePollRef.current);
      wakePollRef.current = null;
    }
    if (wakeChannelRef.current) {
      void supabase.removeChannel(wakeChannelRef.current);
      wakeChannelRef.current = null;
    }
  }, []);
  ...
  const startWakeUp = useCallback(() => {
    ...
    setWakeError(null);
    setWakeStatus("ringing");
    setWakeRinging(true);
    startRingTone();
    // No answer after 45s (matches wake_up_requests expiry) → soft timeout.
    // A manual "Stop ringing" tap cancels this timer, so the missed-ring push
    // is only sent when the ring genuinely times out unanswered.
    wakeTimerRef.current = window.setTimeout(() => {
      stopRingTone();
      setWakeStatus("noanswer");
      void notifyMissedWakeUp(wakeRequestIdRef.current);
    ...
    void (async () => {
      try {
        ...
        wakePollRef.current = window.setInterval(() => {
          void (async () => {
            try {
              ...
              const st = s?.status;
              if (!st || st === "queued" || st === "initiated" || st === "ringing")
                return;
              // Terminal / answered — stop polling and the 45s fallback timer.
              if (wakePollRef.current) {
                clearInterval(wakePollRef.current);
                wakePollRef.current = null;
              }
              ...
              if (st === "in-progress" || st === "completed") {
                haptics.success();
                setWakeStatus("answered");
                wakeCloseRef.current = window.setTimeout(() => stopWakeUp(), 6000);
              } else {
        ...
        // The call could not even be placed — show the reason, and make sure
        // the missed-ring timer doesn't fire (his phone never rang).
        stopRingTone();
        if (wakeTimerRef.current) {
          clearTimeout(wakeTimerRef.current);
          wakeTimerRef.current = null;
        ...
        setWakeError(
          err instanceof Error ? err.message : "Couldn't start the phone call.",
        );
        setWakeStatus("error");
      }
    })();

    // Records the wake-up request server-side; returns the call UUID.
    void supabase
      .rpc("request_wake_up", {
        p_caller: "habiba",
        p_recipient: "faizan",
        p_caller_display: "Umme Habiba 💗",
      })
      .then(({ data, error }) => {
        if (error || !data) {
          if (error) console.warn("[wake-up] backend not ready yet:", error.message);
          return;
        }
        // LIVE ANSWER STATUS — watch this exact call row via Realtime.
        const requestId = String(data);
        wakeRequestIdRef.current = requestId;
        const ch = supabase
          .channel(`wake-up-${requestId}`)
          .on(
            "postgres_changes",
            {
              event: "UPDATE",
              schema: "public",
              table: "wake_up_requests",
              filter: `id=eq.${requestId}`,
            },
            (payload) => {
              const st = (payload.new as { status?: string } | null)?.status;
              if (!st || st === "ringing") return;
              // First transition wins — stop listening so a later 'ended'
              // (after 'answered') can't overwrite the outcome.
              if (wakeChannelRef.current) {
                void supabase.removeChannel(wakeChannelRef.current);
                wakeChannelRef.current = null;
              }
              ...
              if (st === "answered") {
                ...
                setWakeStatus(st === "declined" ? "declined" : "noanswer");
                // Server-side expiry/failure = a genuinely missed ring →
                // missed-ring push (push_log dedupes vs the local timer path).
                if (st === "expired" || st === "failed") {
                  void notifyMissedWakeUp(requestId);
                }
                wakeCloseRef.current = window.setTimeout(() => stopWakeUp(), 5000);
              }
            },
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              flexShrink: 0,
              flexWrap: "wrap", // lets the (optional) now-playing pill wrap onto its own row
          ...
          >
            ...
            {/* Wake Me Up — urgent ring to Faizan's iPhone (immediately LEFT of music) */}
            <motion.button
              data-testid="chat-wake-toggle"
              onClick={startWakeUp}
              whileTap={{ scale: 0.88 }}
              ...
              style={{
                width: 36,
                height: 36,
                background: wakeRinging ? "rgba(255,255,255,0.5)" : "rgba(255,255,255,0.28)",
                border: "1px solid rgba(255,255,255,0.4)",
                lineHeight: 1,
                color: "white",
                boxShadow: "inset 0 1px 0 rgba(255,255,255,0.3)",
              ...
              aria-label="Wake up Faizan"
              title="Wake Me Up"
            >
              <span aria-hidden style={{ fontSize: 17, lineHeight: 1 }}>⏰</span>
            </motion.button>

            {/* Music — opens the full-screen music page (immediately LEFT of search) */}
            <motion.button
              data-testid="chat-music-toggle"
              onClick={() => {
                haptics.light();
                setMusicOpen(true);
              }}
            ...
            {/* Full-screen music page (portals to <body>; audio itself lives in
                the lib/music.ts singleton so closing this page never stops
                playback and playing never re-renders the chat). */}
            {musicOpen && (
              <MusicPage dark={darkMode} onClose={() => setMusicOpen(false)} />
            )}

            {/* Tiny "now playing" pill — self-subscribed + memoized, renders
                null when no song is selected. Zero impact on chat renders. */}
            <NowPlayingPill onOpen={openMusic} />

            {/* Wake Me Up ringing overlay — portaled to <body> */}
            {wakeRinging &&
              createPortal(
                <div
                  style={{
                    ...
                    alignItems: "center",
                    justifyContent: "center",
                    background: "rgba(201,24,74,0.32)",
                ...
                >
                  <motion.div
                    ...
                    style={{
                      ...
                      boxShadow: "0 18px 60px rgba(201,24,74,0.45), inset 0 1px 0 rgba(255,255,255,0.35)",
                    }}
                  >
                    <motion.div
                      animate={
                        wakeStatus === "ringing"
                          ? { rotate: [0, -14, 12, -10, 8, 0], scale: [1, 1.12, 1] }
                          : wakeStatus === "answered"
                            ? { scale: [1, 1.18, 1] }
                            : { scale: 1 }
                      }
                      ...
                      style={{ fontSize: 56, lineHeight: 1, marginBottom: 14 }}
                      aria-hidden
                    >
                      {wakeStatus === "ringing"
                        ? "📞"
                    ...
                    <p
                      data-testid="wake-status-text"
                      style={{
                        fontFamily: "'Cormorant Garamond', serif",
                        fontSize: wakeStatus === "answered" ? 20 : 17,
                        fontWeight: wakeStatus === "answered" ? 600 : 400,
                    ...
                    >
                      {wakeStatus === "ringing" ? (
                        ...
                        "Faizan declined the call 💔"
                      ) : wakeStatus === "error" ? (
                        <>
                          Couldn't ring his phone ⚠️
                          <br />
                          <span style={{ fontSize: 13, opacity: 0.92, fontStyle: "italic" }}>
                            {wakeError ?? "Something went wrong with the call."}
                          </span>
                        </>
                      ) : (
                        "No answer 😴 — missed-ring alert sent to his phone 📲"
                      )}
                    </p>
                    <motion.button
                      onClick={stopWakeUp}
                      ...
                      aria-label={wakeStatus === "ringing" ? "Stop ringing" : "Close"}
                    >
                      {wakeStatus === "ringing" ? "✕ Stop ringing" : "Close"}
                    </motion.button>
                  </motion.div>
                </div>,
                document.body,
...
function MessageRow({
  ...
  // Perspective-based alignment (like Instagram): messages sent by the
  // CURRENTLY selected persona ("sending as") go RIGHT + PINK; messages from
  // the other person go LEFT + WHITE. Switching persona instantly flips the
  // whole conversation because mySender is reactive state.
  const isOther = msg.sender !== mySender;
  ...
  return (
    <motion.div
      ...
      className={`flex ${isOther ? "justify-start" : "justify-end"} gpu-layer`}
      style={{ borderRadius: 22, transition: "box-shadow 260ms ease" }}
    >
      <div
        ref={bubbleRef}
        className={isOther ? "hb-chat-bubble-other" : "hb-chat-bubble-mine"}
      ...
      >
          ...
          >
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              {msg.replyToId && !repliedExists(msg.replyToId) ? (
                ...
                <ReplyPreviewInBubble snap={msg.replySnapshot} inverse={!isOther} />
              )}
            </div>
          </button>
        )}
        ...
        {msg.type === "voice" && msg.mediaUrl && (
          <VoicePlayer url={msg.mediaUrl} durationMs={msg.durationMs ?? 0} inverse={!isOther} />
        )}
        ...
        >
          ...
          >
            <span style={{ fontWeight: 600, opacity: 0.9 }}>
              {msg.sender === "habiba" ? "Umme Habiba" : "Faizan"}
            </span>
